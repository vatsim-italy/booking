...
<thead>
    <tr>
        ...
        <th scope="col">To</th>
        @if ($event->uses_times)
            <th scope="col"><abbr title="Scheduled Take Off Time">STD</abbr></th>
            <th scope="col"><abbr title="Scheduled Time of Arrival">STA</abbr></th>
        @endif
        <th scope="col">Callsign
        </th>
            ...
            <td class="{{ auth()->check() && auth()->user()->use_monospace_font ? 'text-monospace' : '' }}">
                <span class="text-nowrap">
                    @if ($booking->status === \App\Enums\BookingStatus::BOOKED)
                        <span class="status-dot status-booked" title="Booked"></span>
                    @elseif ($booking->status === \App\Enums\BookingStatus::RESERVED)
                        <span class="status-dot status-reserved" title="Reserved"></span>
                    @elseif ($booking->status === \App\Enums\BookingStatus::PENDING_APPROVAL)
                        <span class="status-dot status-pending" title="Pending approval"></span>
                    @else
                        <span class="status-dot status-available" title="Available"></span>
                    @endif
                    {{ $booking->formatted_callsign }}&nbsp;
                    @if ($booking->is_request_slot && $booking->formatted_callsign)
                        <i class="fas fa-star text-info ml-1 align-middle" title="Custom slot"></i>
                    @endif
                    @if ($booking->turnaroundCS)
                        <i class="fas fa-sync-alt text-info ml-1 align-middle" title="Turnaround available"></i>
                    @endif
                </span>
            </td>
            <td class="{{ auth()->check() && auth()->user()->use_monospace_font ? 'text-monospace' : '' }}">
